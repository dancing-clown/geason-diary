...
TODO: 以下部分的文字部分缺少一些实际的优化案例，后续针对性能测试工具举例进行讲解。
缓存友好型数据结构的核心是 **“连续、紧凑、无冲突”**：

优先连续内存（Vec/vector/ 开放寻址哈希），杜绝链表 / 分散指针；

数据紧凑化 + 缓存行对齐，减少浪费；

多线程场景避免伪共享，填充缓存行；

交易所核心场景（订单簿、行情、持仓）需针对性重构，用数组 / 环形缓冲区 / 开放寻址哈希替代传统结构。

贴合CPU缓存的工作机制，最大化利用空间/时间局部性，减少缓存未命中次数（缓存未命中会导致CPU等待内存数据，延迟是命中的100 - 1000倍）。

CPU缓存分为 L1（最快，核心私有）、L2（核心私有）、L3（多核共享）,特性如下：

- 缓存行(Cache Line): 最小单位是64，读一个int，会把相邻的63字节都加载到内存
- 空间局部性：连续内存访问的数据大概率被缓存，命中率高
- 时间局部性：近期访问过的数据，大概率会被再次访问，缓存会保留
- 伪共享：多个线程修改同一缓存行的不同变量，导致缓存行频繁失效

### 优先连续内存布局

CPU 缓存按行预取，连续内存能让一次预取覆盖更多待访问数据，而链式 / 分散内存（链表、散列桶）会导致大量 cache-miss。

|反例|正例|性能差异|
|---|---|---|
|链表（std::list/Rust LinkedList）|动态数组（std::vector/Rust Vec）|订单簿遍历：链表 cache-miss 率 90%+，数组＜10%，遍历速度提升 10-100 倍|
|散列桶（拉链法 HashMap）|开放寻址哈希表（Rust rustc_hash::FxHashMap）|订单查询：开放寻址连续内存，cache-miss 率降低 80%|
|分散的结构体（含指针 / 堆分配字段）|紧凑结构体（栈分配 / 连续堆内存）|持仓数据访问：紧凑结构缓存命中率提升 70%|

### 数据紧凑化

缩小数据体积：移除冗余字段，用更小数据类型
结构体内存对齐：按缓存行对齐，避免跨缓存行

### 避免伪共享

多线程修改同一缓存行的不同变量，导致缓存行频繁失效（CPU缓存一致性协议触发）。
缓存行填充：将共享变量隔在不同的缓存行中，避免冲突

### 时间局部性优化

通过预取（看[性能调优案例](./performance_tuning.md)）和热点数据放栈或近堆处，优先遍历。

### 避免多层间接访问

|反例（多层指针）|正例（扁平化）|性能提升|
|---|---|---|
|`struct Order { User* user; }`|`struct Order { u64 user_id; }`|用户信息单独缓存3-5倍|
|`std::map<u64, Order*>`红黑树+指针|`std::vector<Order>` + 索引数组`u64[] order_ids`|5-10 倍|
